...
function map(obj, objKey, result) {
    prepareValidationParams(result, objKey, obj);
    if (checkRebuild(obj[objKey])) {
      rebuild(obj, objKey, result);
    }
}

function prepareValidationParams(result, key, object) {
    const type = checkArray(object[key]);
    result[key] = `type:${type}, required:true`;
